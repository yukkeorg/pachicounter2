...
| `-new-session` | 続けられるセッションがあっても新しく始める（機種か配線が前回と違えば、指定しなくても新しく始まる） |
...
配信中に手元から補正や新しいセッションの開始を頼むときは、`pachicounter` のサブコマンドを
使います。コアと同じ PC の別の端末や、ショートカットキーから実行します。

```
$ ./pachicounter status                                   # 今の数字を 1 回だけ確かめる
$ ./pachicounter correct normal_rotations +1 取りこぼし   # 補正する（量には符号が必要）
$ ./pachicounter session new -yes 台を入れ替え            # 新しいセッションを始める
```

| サブコマンド | 内容 |
|---|---|
| `status` | 機種・状態・セッション・信号源と、補正できる 6 つのカウンタを表示する |
| `correct <カウンタ> <+量\|-量> [理由]` | カウンタを補正する。カウンタは `status` に出る英字の名前で指定する。値を直接設定する操作はない |
| `session new [-yes] [覚書]` | 新しいセッションを始める。元のセッションには戻れないので、端末からは確認を求め、ショートカットなど端末以外からは `-yes` が必要 |

どのサブコマンドも `-core <アドレス>` で繋ぐコアを変えられます（既定 `127.0.0.1:18888`）。
オプションはカウンタや覚書より前に書きます。終了コードは 0 が成功、1 が失敗、2 が引数の誤りで、
端末以外から実行して失敗したときはデスクトップ通知でも知らせます。
...
`loop:` にすると繰り返し再生します。周回ごとに集計をやり直すので、フロントの見た目を調整するときに便利です。
