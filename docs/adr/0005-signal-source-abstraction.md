...
- **周回の頭と再接続を区別する。** `loop` は 2 周目以降の頭のイベントに、基準イベントに加えて
  `Event.Restart` を立てる。コアは再生のときだけこれを見て集計をやり直す。再接続として扱って
  いたときは、周回をまたいで同じ記録を数え続け、時刻の巻き戻りで玉の推定も止まっていた。
  記録しているセッションでは、ログから再現できないやり直しは受け付けない。
